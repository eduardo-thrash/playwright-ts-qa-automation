...
export class BasicAuthValidations {
  ...
  async expectAccessToRemainAuthorized(): Promise<void> {
    await expect(this.page).toHaveURL(url => url.pathname === '/basic_auth');
    await this.expectSuccessMessage();
  }

  async expectAccessToBeDenied(): Promise<void> {
    expect(requireScenarioValue<number>(this.page, 'basicAuthStatus')).toBe(401);
  }
}
