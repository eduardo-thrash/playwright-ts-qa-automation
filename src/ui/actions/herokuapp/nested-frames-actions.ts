...
export class NestedFramesActions {
  ...
  constructor(page: Page) {
    ...
    this.optionPage = new NestedFramesPage(page);
  }

  async openPage(): Promise<void> {
    await this.homeActions.openOptionPage('Nested Frames');
  }

  async waitForEveryFrame(): Promise<void> {
    await this.waitActions.waitForVisibility(this.optionPage.leftContent);
    await this.waitActions.waitForVisibility(this.optionPage.middleContent);
    await this.waitActions.waitForVisibility(this.optionPage.rightContent);
    await this.waitActions.waitForVisibility(this.optionPage.bottomContent);
  }

  async waitForTopGroup(): Promise<void> {
    await this.waitActions.waitForVisibility(this.optionPage.middleContent);
  }

  async waitForBottomFrame(): Promise<void> {
    await this.waitActions.waitForVisibility(this.optionPage.bottomContent);
  }

  async reloadPage(): Promise<void> {
    await this.navigationActions.reloadPage();
    await this.waitForEveryFrame();
  }
}
