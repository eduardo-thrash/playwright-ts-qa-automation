...
export class FormAuthenticationActions {
  ...
  async openPage(): Promise<void> {
    await this.homeActions.openOptionPage('Form Authentication');
  }

  async enterCredentials(username: string, password: string): Promise<void> {
    await this.inputActions.fillIn(this.optionPage.usernameInput, username);
    await this.inputActions.fillIn(this.optionPage.passwordInput, password);
  }

  async submitLogin(): Promise<void> {
    await this.clickActions.clickOn(this.optionPage.loginButton);
  }

  async login(username = 'tomsmith', password = 'SuperSecretPassword!'): Promise<void> {
    await this.openPage();
  ...
  async logout(): Promise<void> {
    await this.clickActions.clickOn(this.optionPage.logoutButton);
  }

  async reloadSecurePage(): Promise<void> {
