...
export class DynamicLoadingActions {
  ...
  async openExample(example: 1 | 2): Promise<void> {
    await this.navigationActions.navigateTo(`/dynamic_loading/${example}`);
  }

  async startLoading(): Promise<void> {
    await this.clickActions.clickOn(this.optionPage.startButton);
    await this.waitActions.waitForVisibility(this.optionPage.finalContent);
  }

  async startLoadingWithoutWaiting(): Promise<void> {
    await this.clickActions.clickOn(this.optionPage.startButton);
  }

  async viewLoadingControls(): Promise<void> {
    await this.waitActions.waitForVisibility(this.optionPage.loadingIndicator);
  }

  async openEveryExample(): Promise<void> {
    for (const example of [1, 2] as const) {
      await this.openExample(example);
      await this.page.goBack();
    }
  }

  async reopenCurrentExample(): Promise<void> {
