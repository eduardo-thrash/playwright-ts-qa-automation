...
export class DropdownActions {
  ...
  async openPage(): Promise<void> {
    await this.homeActions.openOptionPage('Dropdown');
  }

  async selectOption(label: string): Promise<void> {
    await this.inputActions.selectOption(
      this.optionPage.optionsSelect,
      { 'Option 1': '1', 'Option 2': '2' }[label] ?? label,
    );
  }

  async focusDropdown(): Promise<void> {
    await this.inputActions.focusOn(this.optionPage.optionsSelect);
  }

  async selectNextOptionWithKeyboard(): Promise<void> {
    await this.inputActions.pressKey('ArrowDown');
  }
