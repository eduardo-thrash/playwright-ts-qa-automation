...
export class JavaScriptErrorActions {
  ...
  async openPageAndCaptureErrors(): Promise<void> {
    const errors: string[] = [];
    this.page.on('pageerror', error => errors.push(error.message));
    await this.navigationActions.navigateTo('/javascript_error');
    await this.optionPage.explanatoryContent.waitFor({ state: 'visible' });
    setScenarioValue(this.page, 'javascriptPageErrors', errors);
  }

  async reloadAndCaptureErrors(): Promise<void> {
    const errors: string[] = [];
    this.page.on('pageerror', error => errors.push(error.message));
